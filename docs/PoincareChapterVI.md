...
| §§97–98 | Decide which candidates are admissible and which singularity lies on the boundary of the Laurent annulus | `ChapterVIDAdmissibility.lean` reconstructs the concrete §97 D calculation for `a=-1`, `c=3`, `τ=1/100`, `β=2`: exact branch calculus gives the unique exterior endpoint, exact exponential bounds give both continuations through B, and the terminal configuration is inside/inside/outside. It parameterizes the inside and outside descendants by Poincaré's actual common complex `z`, lifts them continuously to the `x^(1/3)` coordinate, and proves pointwise that both are zeros of his literal collision factor and coalesce at D. `ChapterVIDRootCoordinates.lean` verifies Poincaré's exact change back to `t`, reconstructs `y=ζt`, and proves both paths are zeros of the transformed literal source radicand. Together with `ChapterVIWindingObstruction.lean`, it proves that no `C²` closed contour family starting at the unit circle can keep that radicand nonzero through the radial continuation to D | Place the forced singularity's middle cycle in the prepared local chart and connect it to the exact three-arc decomposition. The general §98 Riemann-surface continuation remains only sketched by Poincaré and is not claimed |
| §99 | Localize at a pinch and prepare the double zero as `ψ=((t-h)²+k)ψ₁` | `ChapterVISourceCoordinates.lean` constructs Poincaré's literal convergent `ψ(z,t)` and splits it as `H H₀`. `ChapterVIDoubleZero.lean` reduces exact fiber order two to four source checks, all discharged for the certified point D by `ChapterVIDFiberDerivative.lean`. `ChapterVIAnalyticCriticalCenter.lean` constructs the analytic critical center. `FiberHadamard.lean` and `ChapterVIAnalyticCentering.lean` divide `ψ(z,h(z)+u)-ψ(z,h(z))` twice and produce a jointly analytic nonvanishing unit. `ChapterVIParametricMorse.lean` restores the unsubtracted critical value: the analytic coordinate `v=u√U(z,u)` has an analytic local inverse and gives exactly `ψ(z,h(z)+u(z,v))=ψ(z,h(z))+v²`. `ChapterVIMorseAmplitude.lean` proves that the inverse Jacobian is analytic and nonzero and pulls the full analytic numerator and differential into Poincaré's `θ(z,v)dv/root(k(z)+v²)` form. `ChapterVICriticalParameter.lean` constructs the `k` coordinate conditional on transversality; `ChapterVIDTransversality.lean` proves `k'(z_D)≠0` from the exact source factors and makes the resulting `k+v²` chart unconditional. `ChapterVIPrincipalIntegrand.lean` constructs the local cubic root of `z`, formalizes the actual §94 principal numerator, and proves its Morse amplitude nonzero. `ChapterVIDGlobalLocalBridge.lean` proves that the explicit global endpoint maps to D, differs from the selected local lift by a cube-root deck transformation, and itself has analytic order two. `ChapterVICycleDecomposition.lean` proves the exact three-arc localization. `ChapterVIJointPreparation.lean` identifies the singular-fiber primitive `A(0) log u+Q(u)` with `A(0)≠0`. | Place the middle arc of the global source cycle in the selected local branch chart and prove analyticity of the nonlocal remainder |
