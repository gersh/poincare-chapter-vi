...
| §§97–98 | Decide which candidates are admissible and which singularity lies on the boundary of the Laurent annulus | The concrete inside and outside descendants, common parameter, transformed literal-radicand zeros, and winding obstruction are proved. `ChapterVIDRadialContour.lean` now constructs an actual closed contour homotopy starting at the unit circle, staying strictly between the two pole radii before D, and reaching their common collision at its final half-turn. | Lift this explicit contour family to the compatible square-root sheet, establish the smooth local parameterization needed for contour transport, and identify its middle arc with the proved local vanishing cycle. The general §98 Riemann-surface continuation remains only sketched by Poincaré and is not claimed |
