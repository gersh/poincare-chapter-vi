...
| §99 | Localize at a pinch and prepare the double zero as `ψ=((t-h)²+k)ψ₁` | `ChapterVISourceCoordinates.lean` constructs Poincaré's literal convergent `ψ(z,t)` and splits it as `H H₀`. `ChapterVIDoubleZero.lean` reduces exact fiber order two to `H=0`, `H'=0`, `H''≠0`, and `H₀≠0`; `ChapterVIDFiberDerivative.lean` discharges all four checks for the certified concrete `D` point. `ChapterVIAnalyticCriticalCenter.lean` applies the complex IFT to the actual radicand and constructs the analytic center `h(z)` with `∂ₜψ(z,h(z))=0`. `ChapterVIAnalyticCentering.lean` constructs the convergent centered germ, proves it and its first fiber derivative vanish along the parameter axis, and proves its second fiber derivative at D is nonzero. `ChapterVIWeierstrass.lean` proves preparation for nested **formal** power series; `ChapterVIAnalyticPreparation.lean` turns convergent prepared factors into the actual local inverse square-root branch and contour transport. `ChapterVIPinchModel.lean` proves the parameter-dependent logarithmic limit | Prove the analytic square-division/preparation theorem for the centered germ, place the actual cycle in the branch chart, and control the nonlocal remainder |
