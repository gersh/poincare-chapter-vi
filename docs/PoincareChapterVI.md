...
- deriving the logarithmic decomposition from the contour pinch and proving that its regular
  amplitude factors extend analytically beyond the common boundary circle;
...
| §100 | Integrate the prepared local model to obtain `Φ₂+Φ₃ log(z-z₀)` and apply Darboux | `ChapterVIDarbouxTransfer.lean` derives Taylor coefficients from equality of analytic germs, proves larger-disk analytic remainders disappear after normalization, and handles a full varying amplitude by writing `G(z)=G(z₀)+(1-z/z₀)H(z)` and applying a proved weighted-convolution estimate; `ChapterVIDarbouxSpectrum.lean` recovers all equal-modulus bases | Derive the logarithmic expansion of the actual integral and the larger-disk analyticity of `H`, uniformly in the orbital parameters |
...
`H`. Reaching it requires proving the function-level logarithmic decomposition, nonzero leading
amplitudes, common boundary radius, two-coordinate dependence, and that analytic extension.
...
the boundary circle. What remains is to establish that regular-factor decomposition for the
