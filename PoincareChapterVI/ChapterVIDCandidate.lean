...
/-- The compiled sign check used by the second-order collision certificate.  Its interval has
width `10⁻¹²`, enough to determine the orientation of the double zero at `D`. -/
theorem chapterVID_ultrafine_endpoint_certificate :
    0 < chapterVIDScaledEval (-26865395705) 1000000000000 ∧
      chapterVIDScaledEval (-26865395704) 1000000000000 < 0 := by
  verified_decide
...
theorem chapterVIDPolynomial_ultrafine_left_pos :
    0 < chapterVIDPolynomial (-26865395705 / 1000000000000) := by
  have hcert : (0 : ℝ) <
      (chapterVIDScaledEval (-26865395705) 1000000000000 : ℤ) := by
    exact_mod_cast chapterVID_ultrafine_endpoint_certificate.1
  have hs := chapterVID_scaled_eval_sound (-26865395705) 1000000000000 (by norm_num)
  norm_num only [Int.cast_ofNat] at hs
  nlinarith

theorem chapterVIDPolynomial_ultrafine_right_neg :
    chapterVIDPolynomial (-26865395704 / 1000000000000) < 0 := by
  have hcert :
      ((chapterVIDScaledEval (-26865395704) 1000000000000 : ℤ) : ℝ) < 0 := by
    exact_mod_cast chapterVID_ultrafine_endpoint_certificate.2
  have hs := chapterVID_scaled_eval_sound (-26865395704) 1000000000000 (by norm_num)
  norm_num only [Int.cast_ofNat] at hs
  nlinarith

/-- The compiled `10⁻¹²` root bracket used to orient the second-order terminal trace. -/
theorem chapterVIDRoot_ultrafine_mem :
    chapterVIDRoot ∈
      Set.Icc (-26865395705 / 1000000000000) (-26865395704 / 1000000000000) := by
  constructor
  · by_contra hnot
    have hrootLt : chapterVIDRoot < -26865395705 / 1000000000000 := lt_of_not_ge hnot
    have hleftMem : (-26865395705 / 1000000000000 : ℝ) ∈
        Set.Icc (-27 / 1000) (-26 / 1000) := by norm_num
    have hstrict := chapterVIDPolynomial_strictAntiOn chapterVIDRoot_mem hleftMem hrootLt
    rw [chapterVIDRoot_isRoot] at hstrict
    linarith [chapterVIDPolynomial_ultrafine_left_pos]
  · by_contra hnot
    have hrightLt :
        (-26865395704 / 1000000000000 : ℝ) < chapterVIDRoot := lt_of_not_ge hnot
    have hrightMem : (-26865395704 / 1000000000000 : ℝ) ∈
        Set.Icc (-27 / 1000) (-26 / 1000) := by norm_num
    have hstrict := chapterVIDPolynomial_strictAntiOn hrightMem chapterVIDRoot_mem hrightLt
    rw [chapterVIDRoot_isRoot] at hstrict
    linarith [chapterVIDPolynomial_ultrafine_right_neg]
