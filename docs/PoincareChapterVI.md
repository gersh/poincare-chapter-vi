...
| §§97–98 | Decide which candidates are admissible and which singularity lies on the boundary of the Laurent annulus | The concrete inside and outside descendants, common parameter, transformed literal-radicand zeros, and winding obstruction are proved. `ChapterVIDRadialContour.lean` constructs an actual closed contour homotopy starting at the unit circle, staying strictly between the pole radii before D, and reaching their collision. `ChapterVIDGlobalMorseBridge.lean` proves that its exact `u` coordinate is locally analytically equivalent to the prepared Morse `v` coordinate and reconstructs the same local source germ. | Lift the full contour family to the compatible square-root sheet and establish the regularity needed for contour transport. The general §98 Riemann-surface continuation remains only sketched by Poincaré and is not claimed |
...
2. Implement the remaining `ChapterVIDPrincipalThreeArcContinuation` data from the explicit §97
   pole paths and the proved global-to-Morse germ: construct the compatible square-root-sheet
   deformation and prove its two outer arcs have finite limits (hence are `o(-log k)`).
