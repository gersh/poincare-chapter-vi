...
| §95 | Candidate singularities arise when moving singularities of the integrand obstruct contour deformation | `ChapterVIContourTransport.lean` proves equality for a relative `C²` homotopy, derives closedness automatically for holomorphic scalar one-forms, connects to the prepared inverse root, and constructs the canonical affine homotopy inside a convex branch subdomain | Place the actual source paths in a suitable convex/local branch domain, verify their `C²` regularity, construct the vanishing cycle when no such direct domain exists, and prove when deformation is obstructed; modern language suggests Picard–Lefschetz theory |
