...
| §95 | Candidate singularities arise when moving singularities of the integrand obstruct contour deformation | `ChapterVIContourTransport.lean` proves equality of contour integrals for a relative `C²` homotopy inside a branch domain, specializes automatically to holomorphic scalar one-forms, and connects directly to the prepared inverse square root | Construct the actual homotopy/vanishing cycle, prove it remains in the branch chart away from the genuine pinch, and show when such a homotopy is obstructed; modern language suggests Picard–Lefschetz theory |
...
| §99 | Localize at a pinch and prepare the double zero as `ψ=((t-h)²+k)ψ₁` | `ChapterVIWeierstrass.lean` proves the analogous statement for nested **formal** power series; `ChapterVIPinchModel.lean` proves the parameter-dependent complex-amplitude log limit and moving-center translation; `ChapterVIComplexBranch.lean` constructs the joint branch chart; `ChapterVIContourTransport.lean` proves the prepared inverse-root integral is invariant under a checked relative `C²` homotopy in that chart | Analytic Weierstrass preparation for the actual convergent germ, nondegeneracy, verify the branch hypotheses and construct the actual source homotopy, then control the nonlocal remainder |
...
- `ChapterVIPinchModel.lean`: exact integration of the real symmetric quadratic-pinch model,
  ...
  center value. A further theorem performs the exact moving-center substitution from
...
- `ChapterVIComplexBranch.lean`: a compatible holomorphic square-root branch for a prepared
  ...
  open common chart containing a prescribed point or entire cycle family.
- `ChapterVIContourTransport.lean`: a genuine non-affine contour-deformation theorem using
  ...
  boundaries, and specializes to the prepared inverse square-root branch.
