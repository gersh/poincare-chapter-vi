...
| §99 | Localize at a pinch and prepare the double zero as `ψ=((t-h)²+k)ψ₁` | `ChapterVISourceCoordinates.lean` now constructs Poincaré's literal `ψ(z,t)` as a convergent analytic germ and proves its base value is the concrete source radicand. `ChapterVIWeierstrass.lean` proves preparation for nested **formal** power series; `ChapterVIAnalyticPreparation.lean` turns convergent prepared factors into the actual local inverse square-root branch and contour transport. `ChapterVIPinchModel.lean` proves the parameter-dependent logarithmic limit | Prove the selected source point has fiber order exactly two, prove convergence of the formal Weierstrass factors (or formalize analytic WPT), place the actual cycle in the branch chart, and control the nonlocal remainder |
...
- `ChapterVISourceCoordinates.lean`: the exact exponential Kepler map and derivative, canonical
  ...
  denominator and critical-point exclusion explicitly.
