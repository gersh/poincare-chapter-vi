...
to interpret all five summands as one local continued square-root sheet.  Connecting this local
lift to the original §94 germ requires a jointly moving cycle; it cannot be replaced by the
current-parameter literal unit circle, as the winding theorem in `ChapterVIDRadialContour` shows.
-/
