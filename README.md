...
- `ChapterVILeanCompCertNonzeroGrid.lean` provides the reusable compiled boundary for those two
  ...
  The previously certified terminal radial box also supplies `outer-local` directly. Collision
  reciprocals and powers are now generated by checked subtraces, while fixed proved rectangles
  supply `Y(D)` and the rational coefficients. The executable 261-cell reference campaign checks
  42,282 operations per side. Every arithmetic claim passes; exactly 171 endpoint-adjacent cells
