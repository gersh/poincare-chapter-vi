...
theorem shard_admissible_initial_24 :
    Admissible (batchClaims (referenceShardOperations .initial ⟨24, by decide⟩)) := by
  refine ⟨?_, ?_, ?_⟩
  · decide +kernel
  · decide +kernel
  · decide +kernel
...
theorem shard_allHold_initial_24 :
    ∀ claim ∈ batchClaims
      (referenceShardOperations .initial ⟨24, by decide⟩), claim.Holds := by
  decide +kernel
