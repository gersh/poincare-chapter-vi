...
- `Section103/Geometry.lean` and `Section103/RuppertCertificate.lean`: exact derivation of the
  affine coefficient table, kernel-checked modular inverse, and lift of the nonzero minor through
  Gaussian integers to full column rank over `ℂ`.
...
`ℂ`. `ChapterVIRuppert` verifies the quotient-rule identity showing that a proper factor produces
a Ruppert-kernel vector. The remaining algebraic tasks are the precise bounded-degree theorem
turning this rank statement into absolute irreducibility and the projective Bézout and local
intersection-multiplicity theorems. Those steps, rather than the finite rank calculation, now form
the gap in the repair of Poincaré's jump from "more than 36 intersections" to "the curves
coincide."
