...
| §96 | Algebraic equations for candidate singularities | `ChapterVISingularityAlgebra.lean` identifies the concrete Laurent radicand with `(ξ-βη)(ξ₀-β₀η₀)`, proves that clearing `2xy` gives the product of the general collision cubics, and verifies both-eccentricity half-angle reductions. `ChapterVISourceCoordinates.lean` proves the exponential Kepler substitution locally analytically invertible, identifies `z=exp(ial)exp(icl')`, tracks the integer-power inverse branch, and constructs the literal analytic germ `ψ(z,t)` with `exp(il)=tᶜ` | Relate the selected admissible algebraic point to the precise local branch and prove its fiber double-zero/nondegeneracy conditions |
...
| §99 | Localize at a pinch and prepare the double zero as `ψ=((t-h)²+k)ψ₁` | `ChapterVISourceCoordinates.lean` now constructs Poincaré's literal `ψ(z,t)` as a convergent analytic germ, splits it exactly as `H H₀`, and proves both pulled-back factors analytic. `ChapterVIDoubleZero.lean` proves that the four finite checks `H=0`, `H'=0`, `H''≠0`, and `H₀≠0` imply exact fiber order two for the complete analytic germ. `ChapterVIWeierstrass.lean` proves preparation for nested **formal** power series; `ChapterVIAnalyticPreparation.lean` turns convergent prepared factors into the actual local inverse square-root branch and contour transport. `ChapterVIPinchModel.lean` proves the parameter-dependent logarithmic limit | Certify those four checks for Poincaré's selected admissible source point, prove convergence of the formal Weierstrass factors (or formalize analytic WPT), place the actual cycle in the branch chart, and control the nonlocal remainder |
